...
def rhi_timeseries_all(dat):
    ...
    for i,f in enumerate(flno):
        ...
        time, rhi = dat_flash['TIME']/3600.+5.75, dat_flash['FLH2O']/dat_flash['SATPPM']
        ax1.plot(time,rhi,'.',color=c[0],label="FLASH")
        
        time, rhi = dat_clr_fish['TIME']/3600.+5.75, dat_clr_fish['FIH2O']/dat_clr_fish['SATPPM']
        ax1.plot(time,rhi,'.',color=c[1],label="clear-sky FISH")
        
        time, rhi = dat_chiwis['TIME']/3600.+5.75, dat_chiwis['H2O']/dat_chiwis['SATPPM']
        ax1.plot(time,rhi,'.',color=c[2],label="ChiWIS")
        
        time, rhi = dat_chiwis_oor['TIME']/3600.+5.75, dat_chiwis_oor['H2O']/dat_chiwis_oor['SATPPM']
        ax1.plot(time,rhi,'.',color='grey')
    ...
    fig.tight_layout()
    plt.savefig("./Paper-Figures/supp-rhi-timeseries.png",dpi=300)
    plt.show()