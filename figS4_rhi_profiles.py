...
def rhi_profiles_all(dat):
    ...
    for i,f in enumerate(flno):
        ...
        ax1.plot(dat_flash['FLH2O']/dat_flash['SATPPM'],dat_flash['PT'],'.',color=c[0],label="FLASH")
        ax1.plot(dat_clr_fish['FIH2O']/dat_clr_fish['SATPPM'],dat_clr_fish['PT'],'.',color=c[1],label="clear-sky FISH")
        ax1.plot(dat_chiwis['H2O']/dat_chiwis['SATPPM'],dat_chiwis['PT'],'.',color=c[2],label="ChiWIS")
        ax1.plot(dat_chiwis_oor['H2O']/dat_chiwis_oor['SATPPM'],dat_chiwis_oor['PT'],'.',color='grey')
    ...
    fig.tight_layout()
    plt.savefig("./Paper-Figures/supp-rhi-profiles.png",dpi=300)
    plt.show()