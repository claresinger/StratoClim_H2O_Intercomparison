...
def rhi_pt_by_pt_whist6(dat):
    ...
    for a,ax in enumerate(axused):
        ...
        if a < 3:
            ...
            ax.text(1.37,0.05,"N={}".format(len(x[mask])))
    ...
    plt.rcParams.update({"font.size":22})
    plt.savefig("./Paper-Figures/fig7-scatter-rhi-hist6.png",dpi=300,bbox_inches="tight")
    plt.show()
...
def rhi_pt_by_pt_whist_oor(dat):
    ...
    for a,ax in enumerate(axused):        
        ...
        if a < 2:
            ...
            ax.text(1.35,0.05,"N={}".format(len(x[mask])))
        ...
        if a == 0:
            ...
            p = np.random.permutation(len(x))
            x, y, fi = x[p], y[p], fi[p]
            ylabel = r"clear-sky FLASH RH$_{ice}$"
        ...
        if a == 2:
            ...
            xlabel = r"clear-sky ChiWIS RH$_{ice}$"
            ylabel = r"clear-sky FLASH RH$_{ice}$"
